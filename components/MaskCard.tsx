    ...
    <>
      ...
      >
        ...
        <div>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--t-sm)', lineHeight: 'var(--lh-normal)', color: 'var(--ink-mid)', margin: '0 0 16px' }}>
            {mask.tagline}
          </p>
          ...
          <button
            ...
            style={{
              ...
              color: mask.color, background: `rgba(${mask.glowRgb},.10)`,
              border: `1px solid rgba(${mask.glowRgb},.40)`,
              borderRadius: 'var(--r-sm)', padding: '9px 16px',
              cursor: 'pointer',
              transition: 'background var(--dur-2) var(--ease-out), box-shadow var(--dur-2) var(--ease-out)',
            }}
            onMouseEnter={e => {
              e.currentTarget.style.background = `rgba(${mask.glowRgb},.20)`
              e.currentTarget.style.boxShadow = `0 0 12px rgba(${mask.glowRgb},.35)`
            }}
            onMouseLeave={e => {
              e.currentTarget.style.background = `rgba(${mask.glowRgb},.10)`
      ...
      {tryOn && <TryOnModal mask={mask} onClose={() => setTryOn(false)} />}
    </>
  )
}
